...
DTB_IMG=${KERNEL_DIR}/arch/arm/boot/dts/s5p4418-navi_ref-rev01.dtb
...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_KERNEL}" == "true" ]; then
	build_kernel ${KERNEL_DIR} ${TARGET_SOC} ${BOARD_NAME} s5p4418_navi_ref_nougat_defconfig ${CROSS_COMPILE}
	test -d ${OUT_DIR} && \
...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_ANDROID}" == "true" ]; then
	...
	test -f ${DEVICE_DIR}/app.te && cp ${DEVICE_DIR}/domain.te ${TOP}/system/sepolicy
